...
class Modelo(threading.Thread):
    ...
    def isOnline(self):
        try:
            resp = requests.get(f'https://strip.chat/api/front/v2/models/username/{self.modelo}/cam').json()
            hls_url = ''
            if 'cam' in resp.keys():
            ...
            if len(hls_url):
                ...
                streamlink.exceptions.StreamError) as e: 
            with open('log.log', 'a+') as f:
                f.write(f'\n{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} EXCEPTION: {e}\n')
            return False  # Trả về False để đánh dấu là offline
...
#         self.stop_event.set()
def process_postprocessing_queue():
    while postprocessing:
        file = postprocessing.pop(0)  # Lấy file đầu tiên và xóa khỏi danh sách

        def target():
            Utils.repair_mp4_file_ffmpeg(file)

        thread = threading.Thread(target=target)  # Tạo luồng mới
        thread.start()  # Bắt đầu xử lý trong luồng riêng
...
def startRecording(recordingList):
    ...
    for file in postprocessing:
        ...
        # output_file = Utils.repair_mp4_file_ffmpeg(file)
        thread = threading.Thread(target=target)
        thread.start()  # Không cần daemon hóa thread
    postprocessing.clear()
    processing_thread = threading.Thread(target=process_postprocessing_queue)
